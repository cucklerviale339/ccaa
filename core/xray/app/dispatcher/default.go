...
type DefaultDispatcher struct {
	...
	Counter      sync.Map
	SameTagOutbound sync.Map // map[string]bool
	LinkManagers sync.Map // map[string]*LinkManager
}

func (d *DefaultDispatcher) SetSameTagOutbound(tag string, enabled bool) {
	if tag == "" {
		return
	}
	if enabled {
		d.SameTagOutbound.Store(tag, true)
		return
	}
	d.SameTagOutbound.Delete(tag)
}

func (d *DefaultDispatcher) CanUseSameTagOutbound(tag string) bool {
	if tag == "" {
		return false
	}
	enabled, ok := d.SameTagOutbound.Load(tag)
...
func (d *DefaultDispatcher) routedDispatch(ctx context.Context, link *transport.Link, destination net.Destination, l *limiter.Limiter, protocol string) {
	...
	if handler == nil {
		if sessionInbound != nil && d.CanUseSameTagOutbound(inTag) {
			if h := d.ohm.GetHandler(inTag); h != nil {
				handler = h
				errors.LogInfo(ctx, "taking same-tag outbound [", inTag, "] for [", destination, "]")
			}
		}
	}
