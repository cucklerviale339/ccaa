...
func (c *Xray) AddNode(tag string, info *panel.NodeInfo, config *conf.Options) error {
	...
	if c.dispatcher != nil && config.XrayOptions != nil {
		c.dispatcher.SetSameTagOutbound(tag, config.XrayOptions.EnableSameTagOutbound)
	}
...
func (c *Xray) DelNode(tag string) error {
	if c.dispatcher != nil {
		c.dispatcher.SetSameTagOutbound(tag, false)
	}
