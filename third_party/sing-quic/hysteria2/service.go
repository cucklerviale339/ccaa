...
func (s *serverSession[U]) handleStream(stream *quic.Stream) error {
	...
	s.handler.NewConnectionEx(auth.ContextWithUser(s.ctx, s.authUser), &serverConn{
		Stream:    stream,
		localAddr: s.quicConn.LocalAddr(),
	}, M.SocksaddrFromNet(s.quicConn.RemoteAddr()).Unwrap(), M.ParseSocksaddr(destinationString).Unwrap(), nil)
	return nil
}
